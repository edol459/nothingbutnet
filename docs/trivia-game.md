...
and is a faster, catchier loop. Generated by `qe.generate_thisorthat` across three axes:
- **Season stat** — "Who averaged more rebounds in 2018-19?" (top-15 of a leaderboard; intuitive
  box stats + shooting %s only — `_TOOT_SEASON`).
- **Career total** — "Who has more career points?" (`SUM(stat·gp)`, nearby ranks for a fair call).
- **Accolades** — "Who won more MVPs?" / "Who made more All-Star teams?" (counts from `awards[]`).

**Correctness guard:** career/accolade axes only use players who **debuted in 1997-98+** (our data
starts 1996-97, so earlier players have truncated totals → wrong answers). Season axis is unaffected.
